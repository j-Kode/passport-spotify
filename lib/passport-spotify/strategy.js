...
const getUserProfile = async (accessToken, url) => {
  try {
    ...
    if (userData.status === 200) {
      console.log("success");
      await getUserProfile(accessToken, url);
    }
    return userData;
  } catch (e) {
    console.log(e.response);
    if (e.response.status === 429) {
      const minimumWaitToRetry = e.response.headers["retry-after"];
      console.log(
        `Rate limited, retrying again after ${minimumWaitToRetry} seconds`
      );
      await new Promise((resolve) =>
        setTimeout(resolve, minimumWaitToRetry * 1000)
      );
      return getUserProfile(accessToken, url);
    } else {
      throw e;
    }
  }
...
Strategy.prototype.userProfile = function (accessToken, done) {
  ...
  getUserProfile(accessToken, this._userProfileURL)
    .then((body) => {
